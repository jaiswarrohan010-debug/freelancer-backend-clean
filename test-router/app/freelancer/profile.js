...
export default function FreelancerProfileScreen() {
  ...
  // Debug function to manually check profile image
  const debugProfileImage = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('🔍 DEBUG: profileImage:', profile.profileImage);
        console.log('🔍 DEBUG: isVerified:', profile.isVerified);
        console.log('🔍 DEBUG: verificationStatus:', profile.verificationStatus);
        
        if (profile.profileImage) {
          const baseUrl = API_BASE_URL.replace(/\/api$/, '');
          const imgUrl = profile.profileImage.startsWith('http') ? profile.profileImage : `${baseUrl}${profile.profileImage}`;
          console.log('🔍 DEBUG: Constructed image URL:', imgUrl);
  ...
  // Function to fix Firebase UID issue
  const fixFirebaseUid = async () => {
    try {
      const firebaseUser = auth().currentUser;
      if (!firebaseUser) {
      ...
      console.log('🔍 FIX UID: Firebase UID:', firebaseUser.uid);
      console.log('🔍 FIX UID: User ID:', userId);
      
      // Try to update the user's firebaseUid directly
      const response = await fetch(`${API_BASE_URL}/users/${userId}/fix-firebase-uid`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.log('🔍 FIX UID: Firebase UID updated:', result);
        Alert.alert('Success', 'Firebase UID updated successfully!');
      } else {
        const error = await response.text();
        console.error('🔍 FIX UID: Error updating Firebase UID:', error);
        Alert.alert('Error', 'Failed to update Firebase UID');
      }
    } catch (error) {
      console.error('🔍 FIX UID: Error fixing Firebase UID:', error);
      Alert.alert('Error', 'Failed to fix Firebase UID');
    }
  };

  // Function to manually fix profile image (for testing)
  const fixProfileImage = async () => {
    try {
      ...
      console.log('🔍 FIX: Firebase UID:', firebaseUser.uid);
      console.log('🔍 FIX: User ID:', userId);
      
      // First, let's try to update the user's firebaseUid to fix the auth issue
      const updateFirebaseUidResponse = await fetch(`${API_BASE_URL}/users/${userId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!updateFirebaseUidResponse.ok) {
        console.log('🔍 FIX: Could not update firebaseUid, trying direct approach');
      }
      
      // Use a sample profile image URL for testing
      const testProfileImageUrl = 'https://freelancer-backend-jv21.onrender.com/images/profile_686cc4d0ca244c216782d45e_1752833398885.jpg';
      
      // Try the profile image update
      const response = await fetch(`${API_BASE_URL}/users/${userId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('🔍 FIX: Profile image updated:', result);
        Alert.alert('Success', 'Profile image updated successfully!');
        
        // Refresh the profile image
        const baseUrl = API_BASE_URL.replace(/\/api$/, '');
        const imgUrl = testProfileImageUrl.startsWith('http') ? testProfileImageUrl : `${baseUrl}${testProfileImageUrl}`;
        setProfileImage(imgUrl);
      } else {
        const error = await response.text();
        console.error('🔍 FIX: Error updating profile image:', error);
        
        // If Firebase auth fails, try a direct approach without auth
        console.log('🔍 FIX: Trying direct approach without Firebase auth...');
        const directResponse = await fetch(`${API_BASE_URL}/users/${userId}/fix-profile-image`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (directResponse.ok) {
          ...
          console.log('🔍 FIX: Profile image updated (direct):', result);
          Alert.alert('Success', 'Profile image updated successfully!');
          ...
          console.error('🔍 FIX: Direct approach also failed:', directError);
          Alert.alert('Error', 'Failed to update profile image');
        }
      }
    } catch (error) {
      console.error('🔍 FIX: Error fixing profile image:', error);
      Alert.alert('Error', 'Failed to fix profile image');
    }
  };
      ...
      if (id) {
        try {
          ...
          if (response.ok) {
            ...
            console.log('🔍 Profile debug - verificationStatus:', profile.verificationStatus);
            ...
            if (profile.isVerified && profile.profileImage) {
              ...
              setProfileImage(imgUrl);
            } else if (profile.verificationStatus === 'approved' && profile.profileImage) {
              // Also check for approved status
              const baseUrl = API_BASE_URL.replace(/\/api$/, '');
              const imgUrl = profile.profileImage.startsWith('http') ? profile.profileImage : `${baseUrl}${profile.profileImage}`;
              console.log('🔍 Profile image URL (approved status):', imgUrl);
              setProfileImage(imgUrl);
            } else {
              console.log('🔍 No profile image - isVerified:', profile.isVerified, 'verificationStatus:', profile.verificationStatus, 'profileImage:', profile.profileImage);
    ...
    >
      ...
      >
        ...
        <View style={styles.debugButtonsContainer}>
          <TouchableOpacity style={styles.debugButton} onPress={debugProfileImage}>
            <Text style={styles.debugButtonText}>🔍 Debug Profile Image</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.debugButton, styles.fixButton]} onPress={fixProfileImage}>
            <Text style={styles.debugButtonText}>🔧 Fix Profile Image</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.debugButton, styles.uidButton]} onPress={fixFirebaseUid}>
            <Text style={styles.debugButtonText}>🔑 Fix Firebase UID</Text>
          </TouchableOpacity>
        </View>
