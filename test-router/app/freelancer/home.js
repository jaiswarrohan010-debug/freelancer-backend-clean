...
export default function FreelancerHomeScreen() {
    ...
    const checkVerificationStatus = async () => {
      try {
        ...
        if (userData) {
          ...
          if (user.needsVerification === false && user.verificationStatus === 'pending') {
            setVerificationSubmitted(true);
            setVerificationLoading(true);
            // Immediately show under review message for better UX
  ...
  useFocusEffect(
    useCallback(() => {
      if (currentUserId && !isCheckingProfile) {
        fetchJobs();
        checkProfileCompletion();
      }
    }, [currentUserId, isCheckingProfile])
  );
